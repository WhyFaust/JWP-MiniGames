...
public void OnClientDisconnect(int client)
{
	...
	if (g_iGameMode == hotpotato)
	{
		...
		{
			PrintToChatAll("\x01[\x02JWP|MG\x01] \x02 %N последний кто держал картошку отключился, выбор нового игрока...", client);
			if (g_hCtTimer != null)
			{
...
public Action OnWeaponEquip(int client, int weapon)
{
	if (g_iGameMode == hotpotato)
	{
		if (weapon == g_iEntityPotato)
		{
			...
			{
				PrintToChat(g_iLastPotatoClient, "\x01[\x02JWP|MG\x01] \x03Вы передали горячую картошку %N", client);
				g_iLastPotatoClient = client;
				SetClientSpeed(client, g_flPotatoSpeed);
				PrintToChat(g_iLastPotatoClient, "\x01[\x02JWP|MG\x01] \x03Вам кинули горячую картошку(deagle). Передайте её другому...");
			}
		}
	}
	else if (g_iGameMode == hidenseek || g_iGameMode == chickenhunt)
